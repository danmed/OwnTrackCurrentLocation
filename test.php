...
<script src="http://maps.google.com/maps/api/js?key=AIzaSyBgon2jUSapL72eT9rzOUVl9bu88ginvXU&sensor=false" type="text/javascript"></script>
...
<script type="text/javascript">
	...
	function initMap() {
		...
		{ // if username has not been spedified and a interval has
			$sql = "SELECT dt, latitude, longitude, tracker_id, FROM  dan_locations WHERE FROM_UNIXTIME(tst) BETWEEN DATE_SUB(NOW(),INTERVAL '".$interval."' DAY) AND NOW()";	
		} 
		else if (!empty($user) && $interval > 0)
		{ // if username is specified and a interval has as well
			$sql = "SELECT tstLocal, lat, lon, username, device FROM  dan_locations WHERE FROM_UNIXTIME(tst) BETWEEN DATE_SUB(NOW(),INTERVAL '".$interval."' DAY) AND NOW() AND username = '".$user."'";	
		}
		else if(!empty($user) && $interval = 0)
		{ // display all records for that usename and 0 as interval -> 0 means ALL records
			$sql = "SELECT tstLocal, lat, lon, username, device FROM  dan_locations WHERE username = '".$user."'";	
		} 
		else
		{ // display all records
			$sql = "SELECT dt, latitude, longitude, tracker_id FROM  dan_locations ";	
		}
